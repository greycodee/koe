...
fn read_file(
    ...
    match app_local_data_dir {
        Ok(path_buf) => {
            app_local_data_dar_path = path_buf.to_str().unwrap().to_string();
        }
        Err(e) => {
            return Err(e.to_string());
        }
    ...
    let pcm_file_path = format!("{}/{}", app_local_data_dar_path, pcm_file_name);
    let mp3_file_path = format!("{}/{}", app_local_data_dar_path, mp3_file_name);
